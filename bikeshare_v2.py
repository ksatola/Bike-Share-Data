...
def get_filters():
    ...
    while True:
        try:
            city = input('Choose city. Type [Chicago, New York City, Washington] and press <enter> : ').lower()
    ...
    while True:
        try:
            month = input('Choose month. Type [All, January, February, March, April, May, June] and press <enter> : ').lower()
    ...
    while True:
        try:
            day = input('Choose day of week. Type [All] or any day name and press <enter> : ').lower()
...
# -------------------------
def display_data(df, step):
    """
    Displays raw data in batches of a defined size.

    Args:
        df - pandas dataframe
        (int) step - number of records displayed at a time
    """
    ...
    while True:
        try:
            show_raw_data = input('Do you want to see raw data? Enter [yes] or anything else if no and press <enter>: ').lower()
            if show_raw_data == 'yes':
                current_index = 0
                while True:
                    # show step number of rows at a time from current_index and 8 columns (for better visibility)
                    print('\n')
                    print(df.iloc[current_index:current_index+step, 1:8])
                    print('\n' + '-'*40)
                    show_more = input('Do you want to see more raw data? Enter [yes] or anything else if no and press <enter>: ').lower()
                    current_index += step
                    if show_more != 'yes':
                        break
            break
